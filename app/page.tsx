...
function parseScannedData(raw: string): ParsedScan {
  try {
    ...
    // Not JSON, attempt URL with query params
    try {
...
export default function HomePage() {
  ...
  const canSubmit = useMemo(() => {
    return Boolean(parsed?.raffleId && walletAddress)
  }, [parsed?.raffleId, walletAddress])
  ...
  const handleEnterRaffle = useCallback(async () => {
    ...
    try {
      ...
      if (parsed.valueWei) {
        overrides.value = parsed.valueWei
      }
      ...
      // Call backend to mark QR as used
